...
const logos = {
    participacion: ` <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
...
electores: `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="841.889px"
height="595.281px" viewBox="0 0 841.889 595.281"
enable-background="new 0 0 841.889 595.281" xml:space="preserve"
class="v-icon__component theme--light"
...
mesas: `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="841.889px"
height="595.281px" viewBox="0 0 841.889 595.281"
enable-background="new 0 0 841.889 595.281" xml:space="preserve"
class="v-icon__component theme--light"
...
function agregarInforme() {
    // Verificar si infoJSON tiene contenido
    if (Object.keys(infoJSON).length !== 0) {
      // Construir el objeto dataInforme
      var nuevoInforme = {
      ...
      var contenidoActual = localStorage.getItem('dataInforme');
  
      // Verificar si ya hay contenido en localStorage
      if (contenidoActual) {
        try {
          // Convertir el contenido a un array de objetos
          var contenidoArray = JSON.parse(contenidoActual);
  
          // Validar si el nuevo informe ya está en el array
          var informeExistente = contenidoArray.find(function(informe) {
            // Comparar los objetos por sus propiedades
            return JSON.stringify(informe) === JSON.stringify(nuevoInforme);
          });
  
          // Si el informe ya existe, no permitir agregarlo
          if (informeExistente) {
            console.warn("El informe ya existe en localStorage. No se agregará.");
            return;
          }
  
          // Agregar el nuevo informe al array
          contenidoArray.push(nuevoInforme);
  
          // Convertir de nuevo a cadena JSON y almacenar en localStorage
          localStorage.setItem('dataInforme', JSON.stringify(contenidoArray));
        } catch (error) {
          console.error("Error al manipular JSON en localStorage:", error);
        }
      } else {
        // Si no hay contenido, crear un nuevo array con el nuevo informe
        var nuevoContenido = [nuevoInforme];
  
        // Convertir a cadena JSON y almacenar en localStorage
        localStorage.setItem('dataInforme', JSON.stringify(nuevoContenido));
      }
    } else {
